...
* `slug` — String (required, unique, used for API reference or routing)
...
* `price_previous` — Number (required, previous price for reference)
* `currency` — Enum: `USD | BDT` (default: USD, payment currency)
...
* `price` — Number (required, package price at that time)
...
* `_id` — ObjectId
* `name` — String (required, unique, e.g., "Stripe", "Paypal")
* `currency` — String (required, 3-letter ISO code)
* `secret` — String (required, gateway secret key)
* `public_key` — String (optional, if required by gateway)
* `description` — String (optional)
...
* `gateway_transaction_id` — String (required, transaction ID from payment gateway)
* `package` — ObjectId (Ref: Package, required)
* `amount_usd` — Number (required)
* `amount_bdt` — Number (required)
* `exchange_rate` — Number (optional, used if conversion required)
* `is_deleted` — Boolean (default: false, soft delete)
* `created_at`, `updated_at` — Auto-generated by Mongoose timestamps
