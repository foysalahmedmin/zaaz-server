...
User account management with authentication and role-based access control.
...
- `password` — String (required, 6-12 characters, hashed with bcrypt, not selected by default)
- `password_changed_at` — Date (optional, auto-updated on password change, not selected by default)
- `role` — Enum: `super-admin | admin | editor | author | contributor | subscriber | user` (required)
- `status` — Enum: `in-progress | blocked` (optional, default: `in-progress`)
- `is_verified` — Boolean (optional, default: false, email verification status)
- `is_deleted` — Boolean (optional, default: false, soft delete flag, not selected by default)
...
- Unique index on email with partial filter for non-deleted users
- Password and sensitive fields are excluded from JSON output
- Email verification is reset when email is changed
- Query middleware automatically excludes deleted users
- Static methods: `isUserExist(_id)`, `isUserExistByEmail(email)`
...
Authentication module (not a database model, but a service module for handling authentication operations).
...
**Note**: Auth module uses the User model but does not have its own database schema. It handles authentication logic and JWT token management.
...
1. **Data Model Reference**:
   - Use the attached data model document (User, Feature, FeatureEndpoint, Package, PackageHistory, PaymentMethod, UserWallet, TokenTransaction, PaymentTransaction, TokenProfit, TokenProfitHistory, Contact).
...
- **User Module** and **Auth Module** are now part of this project. The User model is defined in `src/app/modules/user/user.model.ts` and authentication logic is in `src/app/modules/auth/`. User information can be accessed from `req.user` after authentication middleware.
